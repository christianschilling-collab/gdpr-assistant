...
export default function NewRequesterTypePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage('Requester type created successfully!');
      setTimeout(() => {
        router.push('/admin/requester-types');
      }, 1500);
    } catch (err: any) {
      console.error('Error creating requester type:', err);
      setError(err.message || 'Failed to create requester type');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h1 className="text-3xl font-bold text-gray-900">➕ Create New Requester Type</h1>
            <p className="text-gray-600 mt-2">
              Add a new type of requester (e.g., Customer, Employee, Funeral Home)
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
          ...
          <div className="flex items-center gap-4 pt-6 border-t border-gray-200">
            ...
            >
              {loading ? 'Creating...' : 'Create Requester Type'}
            </button>
            <Link
